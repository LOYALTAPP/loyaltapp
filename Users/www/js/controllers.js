...
.controller('CardCtrl', function($scope, $http, $rootScope){
  ...
  if(self.user.card == null){
    $scope.user.card = "<b>Loading your card ....</b>";
    $http.post("https://qrgen-ontechedge.c9.io/genqr", $scope.user).success(function(data, e){
    //$http.post("http://localhost:8101/genqr", $scope.user).success(function(data, e){
      self.user.card = data;
      localStorage.setItem("user", JSON.stringify(self.user));
      $scope.user.card = data;
    });
  }
...
.controller("LoginCtrl", function($scope, $cordovaBarcodeScanner, $cordovaOauth, $http, $state, loginFactory, $ionicLoading){
...
.service("dataService", function(){
  ...
  this.transactionDetails = 
    { id: 1, balance: 130, title : 'Kalyan Bhel', area : 'Vimannagar', 
        breakup: [{date: (new Date()).toLocaleString(), points : 100, transType : 'Collected'},{date: (new Date()).toLocaleString(), points : 60, transType : 'Collected'},{date: (new Date()).toLocaleString(), points : 30, transType : 'Redeemed'}]  
    }
})